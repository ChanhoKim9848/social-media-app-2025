// Import types for Post and User
import { Post, User } from "@/types";

// Import helper functions to format date and number
        ...
        <View className="flex-1">
          ...
          <View className="flex-row items-center justify-between mb-1">
            ...
            <View className="flex-row items-center flex-wrap">
              ...
              <Text className="text-gray-500 ml-1">
                @{post.user.username} · {formatDate(post.createdAt)}  {/* Username and date */}
              </Text>
            </View>
            ...
            {isOwnPost && (
              <TouchableOpacity onPress={handleDelete}>
                <Feather name="trash" size={20} color="#657786" />  {/* Trash icon */}
              </TouchableOpacity>
            )}
          </View>
          ...
          <View className="flex-row justify-between max-w-xs">
            ...
            <TouchableOpacity className="flex-row items-center" onPress={() => onComment(post)}>
              <Feather name="message-circle" size={18} color="#657786" /> {/* Comment icon */}
              <Text className="text-gray-500 text-sm ml-2">
                {formatNumber(post.comments?.length || 0)} {/* Show number of comments */}
              </Text>
            </TouchableOpacity>

            {/* Retweet/share placeholder */}
            <TouchableOpacity className="flex-row items-center">
              <Feather name="repeat" size={18} color="#657786" /> {/* Retweet icon */}
              <Text className="text-gray-500 text-sm ml-2">0</Text> {/* Hardcoded 0 */}
            </TouchableOpacity>
            ...
            <TouchableOpacity>
              <Feather name="share" size={18} color="#657786" />  {/* Share icon */}
            </TouchableOpacity>
          </View>
        </View>
      </View>
