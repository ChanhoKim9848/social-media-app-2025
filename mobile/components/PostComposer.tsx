...
const PostComposer = () => {
    // fetch everything from the custom hook
  const {
    content,
    setContent,
    selectedImage,
      ...
      <View className="flex-row justify-between items-center mt-3">
        <View className="flex-row">
          <TouchableOpacity className="mr-4" onPress={pickImageFromGallery}>
            <Feather name="image" size={20} color="#73C883" />
          </TouchableOpacity>
          <TouchableOpacity className="mr-4" onPress={takePhoto}>
            <Feather name="camera" size={20} color="#73C883" />
          </TouchableOpacity>
        </View>
        ...
        <View className="flex-row items-center">
          ...
          <TouchableOpacity
            className={`px-6 py-2 rounded-full ${
              content.trim() || selectedImage ? "bg-green-500" : "bg-gray-300"
            }`}
            onPress={createPost}
